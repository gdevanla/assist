...
public class JsonReaderTest extends TestCase{
    ...
    @Test
    public void testReadingFormulaClauses() throws ParseFormatException,
            ContradictionException, IOException {
        when(solver.addClause(VecInt.EMPTY)).thenThrow(
                new ContradictionException());
        try {
        	...
        	assertEquals("Parsing ErrorWrong input json",e.getMessage());
        }
    ...
    @Test
    public void testDecode() throws ParseFormatException,
            ContradictionException, TimeoutException {
        String json = "[[1,-2,3],[1,2,3],[1,-3],[-1,-3],[-1,2,3]]";
        ISolver realSolver = SolverFactory.newDefault();
        JSONReader jsonReader = new JSONReader(realSolver);
        ...
        assertEquals("[1,2,-3]", jsonReader.decode(model));
    }
    
    @Test
    public void testHandleNotHandled() throws Exception {
    	ISolver realSolver = SolverFactory.newDefault();
        JSONReader jsonReader = new JSONReader(realSolver);
        
    	try {
    		jsonReader.handleNotHandled("constraint");
    	} catch (Exception e) {
    		assertEquals("Parsing ErrorUnknown constraint: constraint", e.getMessage());
    	}
    }
