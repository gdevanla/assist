...
public class TestsFonctionnels extends TestCase {
	...
	protected static final String PREFIX = "org.sat4j.core/src/test/testfiles/";
    /**
    ...
    public void testDefaultSolver() {
    	...
    	ISolver expected = factory.defaultSolver();
    	assertEquals(Solver.class,expected.getClass());
    }
    
    public void testNewLight() {
    	ISolver expected = SolverFactory.newLight();
    	assertEquals(Solver.class,expected.getClass());
    }
    
    public void testLightSolver() {
    	SolverFactory factory = SolverFactory.instance();
    	ISolver expected = factory.lightSolver();
    	assertEquals(Solver.class,expected.getClass());
    }
       
    public void testDimacsOutput() {
    	ISolver expected = SolverFactory.newDimacsOutput();
    	assertEquals(DimacsOutputSolver.class,expected.getClass());
    }
    
    public void testInstance() {
    	SolverFactory factory1 = SolverFactory.instance();
    	SolverFactory factory2 = SolverFactory.instance();
    	assertTrue(factory1.equals(factory2));
    }
    
    public void testCreateInstance() throws Exception{
    	
    	Class c 	= SolverFactory.class;
		Method m        = c.getDeclaredMethod("createInstance");
		m.setAccessible( true );
		m.invoke(SolverFactory.class);
		m.invoke(SolverFactory.class);
		
    }
    
    public void testStatistics() {
    	ISolver expected = SolverFactory.newStatistics();
    	assertEquals(StatisticsSolver.class,expected.getClass());
    }
